...
let setUrl = co(function* () {
  console.log("111");
  let config   = new Config(configPath());
  let url = yield prompt("url");
  debug("changing base url");
  config.set("url", url);
...
let signup = co(function*(){
  let config   = new Config(configPath());
  let username = yield prompt("username");
  let password = yield prompt.password("password");
  let repeatPassword = yield prompt.password("repeat password");
  let token;
  debug("attempting to signup");
  console.log({ username: username, password: password, repeatPassword: repeatPassword });
  let client = program.client(config, { username: username, password: password, repeatPassword: repeatPassword });

  try {
    token = yield client.signup();
  }
  catch (error) {
    debug("Error: %s", error.message);
    throw new Error("Failed to authenticate.");
...
program
  .command("signup")
  .description("Register a new user and acquire a new access token")
  .action(handle(signup));
...
program
  .command("set-url")
  .description("Change service_maker base url")
  .action(handle(setUrl));  

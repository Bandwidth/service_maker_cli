...
let listTypes = co(function* () {
  ...
  if(program.verbose){
    ...
    types = _.map(types, function (type) { return [ type.type, type.providers.map(function(provider){return provider.name;}).join(", ") ]; });
...
program.option("-c, --credential <name>=<value>", "credential parameter for command \"service-create\"", parseOption, {});
program.option("-o, --option <name>=<value>", "option for command \"service-create\"", parseOption, {})

program.on("--help", function (command) {
  console.log("  Examples:");
  console.log("");
  console.log("    $ cli types list -v");
  console.log("    $ cli service-create host:oceano -c clientId=do-client-id -c apiKey=do-api-key -o name=myVPS");
});
